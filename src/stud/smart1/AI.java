...
public class AI extends core.player.AI {
    ...
    public Move findNextMove(Move opponentMove) {
        ...
        while (true) {
            //从中心13*13区域随机选
            int index1 = index1Box.get(rand.nextInt(index1Box.size()));
            int index2 = index1Box.get(rand.nextInt(index1Box.size()));
        ...
        while(true){
            int index1 = index1Box.get(361);
            int index2 = index1Box.get(361);
            if(index1 != index2 && this.board.get(index1) == PieceColor.EMPTY && this.board.get(index2) == PieceColor.EMPTY) {
                Move move = new Move(index1, index2);
                this.board.makeMove(move);
                return move;
